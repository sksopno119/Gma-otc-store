...
💸 **Accounts Sold:** {user_info['accounts_sold']}

📅 **Account Created:** {user_info['created_at'][:10]}
🕐 **Last Activity:** {user_info['last_activity'][:16]}
"""
...
"""
    ...
    keyboard = [
        ...
        [
            InlineKeyboardButton("💎 TRC20", callback_data="withdraw_trc20"),
            InlineKeyboardButton("🔸 BEP20", callback_data="withdraw_bep20")
        ],
        [
        ...
        [
            InlineKeyboardButton("📱 Cash App", callback_data="withdraw_cashapp"),
            InlineKeyboardButton("🇮🇳 UPI", callback_data="withdraw_upi")
        ],
        [
            InlineKeyboardButton("🏦 Bank (USA/UK/Canada Only)", callback_data="withdraw_bank")
        ],
        [InlineKeyboardButton("🔙 Main Menu", callback_data="main_menu")]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
⚠️ Please do not send anything until you receive further instructions.
"""
...
async def handle_pin_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # CRITICAL: Check if admin has actually approved the number yet
    if not context.user_data.get('admin_approved'):
        await update.message.reply_text(
            "⏳ **Please wait.** Your number is still being verified by the admin.\n"
            "You cannot enter a PIN yet."
        )
        return WAITING_FOR_ADMIN_APPROVAL
    ...
    with user_data_lock:
        ...
        if user_number in user_data[user_id].get('sold_numbers', []):
             await update.message.reply_text("❌ This number has already been processed.")
             return ConversationHandler.END
...
👤 **User:** {user_id}
💰 **Price:** ${price} USD
📈 **Status:** Approved - Waiting for user's PIN
"""
    ...
    try:
        await context.bot.send_message(
            chat_id=user_id,
            text=f"✅ **Your request has been approved!**\n\nPlease enter the **verification code (PIN)** sent to your Telegram account for number:\n📞 `{context.user_data.get('user_number', 'Unknown')}`\n\n💰 Once verified, ${price} USD will be added to your balance.",
            parse_mode='Markdown'
        )
    except Exception as e:
